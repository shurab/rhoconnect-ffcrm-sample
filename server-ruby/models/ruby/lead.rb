...
class Lead < Rhoconnect::Model::Base
  include Helper

  def initialize(source)
    @base =  "http://rho-fat-free.herokuapp.com/leads"
    #@base =  "http://localhost:3000/leads"
    super(source)
  end
  ...
  def query(params=nil)
    ...
    @result={}
    parsed.each do |item|
      @result[item["lead"]["id"].to_s] = item["lead"]
    end if parsed
  end
  ...
  def create(create_hash)
    ...
    JSON.parse(RestClient.get("#{res.headers[:location]}.json", {:cookies => @cookies}).body)["lead"]["id"]
  end
  ...
  def update(update_hash)
    ...
    RestClient.put("#{@base}/#{obj_id}", {:contact => update_hash}, {:cookies => @cookies})
  end
