...
class Opportunity < Rhoconnect::Model::Base
  ...
  def initialize(source)
    @base =  "http://rho-fat-free.herokuapp.com/opportunities"
    #@base =  "http://localhost:3000/opportunities"
    super(source)
  end
  ...
  def query(params=nil)
    ...
    @result = {}
    parsed.each do |item|
      @result[item["opportunity"]["id"].to_s] = item["opportunity"]
    end if parsed
  end
  ...
  def create(create_hash)
    ...
    JSON.parse(RestClient.get("#{res.headers[:location]}.json", {:cookies => @cookies}).body)["opportunity"]["id"]
  end
  ...
  def update(update_hash)
    ...
    # TODO: get/set properly account for updated opportunity record (now is nil)
    RestClient.put("#{@base}/#{obj_id}",
      {:opportunity => update_hash, :account => { :id => "", :name => ""}}, {:cookies => @cookies})
  end
